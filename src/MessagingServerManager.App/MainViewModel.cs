...
public sealed class ServerRowViewModel : ObservableObject
{
    ...
    public int? LastExitCode { get => exit; set => Set(ref exit, value); }
    ...
    void FinishTelemetry(TimeSpan value) { Uptime = value; Started = DateTime.Now - value; Raise(nameof(RemoteMetricsSummary)); Raise(nameof(RawTelemetry)); Raise(nameof(HasRawTelemetry)); Raise(nameof(ConnectionCount)); Raise(nameof(MessageRateTotal)); Raise(nameof(ConnectionsText)); Raise(nameof(MessageRateText)); Raise(nameof(ActivityTooltip)); }
...
public sealed class MainViewModel : ObservableObject, IDisposable
{
  ...
  new(){Name="Local RV Daemon",ServerType=ServerType.TibcoRendezvous,Enabled=false,Executable="rvdaemon.exe",LaunchMode=LaunchMode.ManagedOptions,LogFilePath="tibrv/local-rvdaemon.log",TibRv=new(){Service=7500,HttpAdministrationPort=7580}}
    ];
    ...
    internal async Task RefreshAllAsync() { foreach (var r in Servers) { try { await RefreshAsync(r); } catch (OperationCanceledException) when (shutdown.IsCancellationRequested) { throw; } catch (Exception ex) { r.LastError = ex.GetBaseException().Message; r.Status = ServerStatus.Failed; r.Health = "Refresh failed: " + r.LastError; try { await LogTelemetryFailureAsync(r, ex); } catch (Exception logException) { Debug.WriteLine($"Could not log refresh failure: {logException.Message}"); } } } LastUpdated = DateTime.Now; Raise(nameof(ServerSummaryText)); }
    ...
    async Task RefreshSelectedTelemetryAsync() { if (Selected is not null) await RefreshAsync(Selected); }
    ...
    async Task StartAsync(ServerRowViewModel r) { var v = ServerValidator.Validate(r.Definition, Servers.Select(x => x.Definition), paths.Resolve); if (!v.IsValid) { MessageBox.Show(string.Join("\n", v.Errors), "Cannot start", MessageBoxButton.OK, MessageBoxImage.Warning); return; } try { r.Status = ServerStatus.Starting; await processes.StartAsync(r.Definition, shutdown.Token); await RefreshAsync(r); await SaveRuntimeAsync(); } catch (Exception ex) { r.LastError = ex.Message; r.Status = ServerStatus.Failed; } RaiseCommands(); }
    ...
    async Task RestartAsync(ServerRowViewModel r) { intentionalExits.Add(r.Definition.Id); r.Status = ServerStatus.Restarting; try { await processes.RestartAsync(r.Definition, shutdown.Token); r.RestartCount++; await RefreshAsync(r); await SaveRuntimeAsync(); } catch (Exception ex) { intentionalExits.Remove(r.Definition.Id); r.LastError = ex.Message; r.Status = ServerStatus.Failed; } RaiseCommands(); }
    ...
    List<string> ValidateBundle(PortableConfigurationBundle bundle) { var errors = ServerValidator.ValidateIdentities(bundle.Servers).ToList(); if (string.IsNullOrWhiteSpace(bundle.Settings.LoggingRootDirectory)) errors.Add("Logging root directory is required."); if (bundle.Settings.MonitoringIntervalSeconds < 1) errors.Add("Monitoring interval must be at least one second."); if (bundle.Settings.MaximumLogLines < 1) errors.Add("Maximum log lines must be at least one."); foreach (var server in bundle.Servers) foreach (var error in ServerValidator.Validate(server, bundle.Servers, paths.Resolve).Errors) errors.Add($"{server.Name}: {error}"); return errors.Distinct().ToList(); }
    ...
    public void Dispose() { if (disposed) return; disposed = true; relativeTimeTimer.Stop(); shutdown.Cancel(); monitoringIntervalChanged.Cancel(); processes.Dispose(); monitoringIntervalChanged.Dispose(); shutdown.Dispose(); }
}

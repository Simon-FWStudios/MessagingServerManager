...
public class InfrastructureTests:IDisposable
{
 ...
 [Fact]public async Task Portable_configuration_round_trips_without_runtime_state(){Directory.CreateDirectory(root);var path=Path.Combine(root,"export.msmconfig.json");var service=new ConfigurationTransferService();var bundle=new PortableConfigurationBundle{Settings=new(){MaximumLogLines=321},Servers=[new(){Name="Exported NATS"}]};await service.ExportAsync(path,bundle);var imported=await service.ImportAsync(path);Assert.Equal(1,imported.SchemaVersion);Assert.Equal(321,imported.Settings.MaximumLogLines);Assert.Equal("Exported NATS",Assert.Single(imported.Servers).Name);var json=await File.ReadAllTextAsync(path);Assert.DoesNotContain("ProcessId",json);}
